...
incexlFunc <- function(pop, nr = NULL, export = FALSE) {
  ...
  flowtab <- rbind(flowtab, funcIEfake("3. LVEF >=45% by echocardiography during the screening epoch, or within 6 months prior to screening visit (in primary inclusion)"))
  ...
  ## Export to Excel
  if (export) {
    write.xlsx(flowtab, paste0("./output/tabs/tabincexc_karen_", Sys.Date(), ".xlsx"),
      rowNames = FALSE
    )
  }
...
### Overall
```{r ietaboverall}
incexlFunc(!is.na(IND_YRS))
```

### Subgroup: Females
```{r ietabf}
incexlFunc(IND_GEN == "Female")
```
