...
logregFunc <- function(filt, filtname) {
  ...
  myor <- function(xvar, filt2 = filt) {
    ...
    cmod <- my.glm.mids(formula(paste0("EFgroup57 == '<=57' ~ ", xvar)),
  ...
  cmodadj <- my.glm.mids(formula(paste0("EFgroup57 == '<=57' ~ ", paste(adjmodvars, collapse = " + "))),
  ...
  write.xlsx(ortaball, paste0("./output/tabs/ortab_", filtname, "_", Sys.Date(), ".xlsx"), rowNames = FALSE)
...
### Overall
```{r logregoverall}
logregFunc(quote(!is.na(IND_YRS)), filtname = "overall")
```

### Female
```{r logregfem}
logregFunc(filt = quote(IND_GEN == "Female"), filtname = "female")
```

### Male
```{r logregmal}
logregFunc(quote(IND_GEN == "Male"), filtname = "male")
```
